...
export function parseChangelog(changelogContent) {
    if (
        ...
        changelogContent.trim().length === 0
    ) {
    ...
    if (matches.length === 0) {
        throw new Error(
            "CHANGELOG must contain at least one version section heading.",
        );
    }
    ...
    for (let index = 0; index < matches.length; index += 1) {
        ...
        sections.push({
            ...
            body: trimBody(
                changelogContent.slice(headingEnd, nextHeadingStart),
            ),
        });
    }
...
export function renderChangelog(parsed) {
    ...
    for (const section of sections) {
        ...
        if (!key) {
            throw new Error(
                "Cannot render changelog section with an empty key.",
            );
        }
...
export function findSectionIndexByKey(sections, key) {
    ...
    return sections.findIndex(
        (section) => normalizeKey(section.key) === normalizedKey,
    );
}
...
export function prepareChangelogForRelease({
    ...
    const promotedBody = ensureCategoryHeadings(
        unreleasedBodyOriginal,
        categories,
    );
    parsed.sections[unreleased.index].body = normalizedScaffold;
...
export function assertChangelogPreparedForRelease({
    ...
    if (
        !hasRequiredCategoryHeadings(unreleased.section.body ?? "", categories)
    ) {
        throw new Error(
...
export function extractSectionBullets(body) {
    ...
    for (const line of (body ?? "").split(/\r?\n/)) {
        ...
        if (
            /^\s{2,}\S+/.test(trimmed) &&
            bullets.get(currentHeading).length > 0
        ) {
