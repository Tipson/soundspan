...
- Vibe is now the online-first My Wave surface on every installation: switching For you, New to me, or Familiar requests the matching server-side ranking policy, starting a Wave enables endless provider continuation, and the page no longer mounts or advertises the legacy local Audio-DNA map. Its focused radio layout keeps one primary launch action, compact direction controls, and current-track feedback while removing the finite seed-count label and horizontal preview shelf that made the endless flow look like a fixed playlist. On phones and tablets, the persistent mini player is the single current-track control surface instead of duplicating the same controls inside Vibe.
