...
- The authenticated shell uses one restrained music stage with compact Home, Vibe, and Library navigation, a persistent global search bar, a quieter playlist sidebar, and an account drawer focused on liked songs, playlists, device downloads, imports, and settings. Search results remain a deep-linkable canvas behind that global input instead of appearing as a duplicate navigation destination. The artwork-aware desktop player and full-screen mobile player keep the primary transport centered, show preference controls predictably, reserve seek-by-seconds actions for long-form audio, and adapt to reduced motion, reduced transparency, high contrast, safe areas, and 320 px screens.
- Home opens with a compact one-tap My Wave action using artwork from the active account feed, keeps everyday music shortcuts nearby, and separates Continue listening from a bounded Made For You set of up to six distinct, playable account-backed or generated mixes.
- Home and Explore are one online-first landing page: the legacy `/explore` entry redirects to Home, provider content is folded into one deduplicated Stations for you row and one New & noteworthy row, empty rows stay hidden, regional spillover is suppressed, and one Calm, Energy, Focus, Workout, Favorites, or Forgotten context grid opens the matching personalized Vibe.
