...
- Load bounded playback/collection signals with their selected track fields in PostgreSQL, reducing seven round trips to four and avoiding repeated ORM relation assembly while preserving account scope, parent limits and signal ordering.
...
- Split deployments can apply a hardened single-host Compose overlay that binds only the frontend to the selected LAN address, keeps API/database/cache ports on loopback, preserves provider state and tuned YouTube Music spool/cache limits, keeps a read-only media library for core runtimes, and shares only the dedicated remote-analysis spool with the required write permissions.
- Split deployments include a validated PostgreSQL backup helper targeting the standalone database container while retaining the existing locked, checksummed, retention-bounded backup format.
- Hybrid v2 now gives Home, My Wave, Made For You, and Similar Tracks one canonical recommendation boundary with account-scoped multi-centroid taste, hard dislikes, persistent one/seven-day repeat cooldowns, deterministic recent-history backfill, bounded per-lane reserves, scalar audio plus bounded active-space DCLAP text-mood scoring, MMR diversity, artist/album caps, deterministic exploration, and playable provider fallback. Only rendered rows become exposures, so opening Home cannot exhaust the next Wave request. The default shadow mode serves baseline-v1 while persisting paired hybrid-v2 generations, source attribution, latency, exposures, and playback outcomes; a read-only CLI reports playability, completion, skips, repeats, artist diversity, mean/p95 latency, and paired overlap before activation.
- Hybrid v2 can use a stable account-percentage canary, an explicit unfamiliar-music quota, a time-decayed profile of the last thirty session actions, and coarse local-time, device, and surface context. The evaluator reports viewport-confirmed impressions, participating accounts, and separate ISRC, MBID, fingerprint, scalar-analysis, and DCLAP coverage instead of treating every server-generated card as a user exposure.
- An optional ListenBrainz candidate adapter adds collaborative-filter MBIDs and LB Radio tags behind one total deadline, bounded last-good caching, a circuit breaker, abortable globally limited playable resolution, partial-resolution tolerance, and YouTube Music fallback. Optional remote hot-set analysis globally deduplicates canonical recordings, atomically enforces a daily budget and one active lease per canonical recording, streams at most 64 MiB into an owned temporary spool under an abortable absolute fifteen-minute deadline, tracks DCLAP and Essentia completion independently, uses fenced PostgreSQL leases for rolling-deploy-safe hand-off, terminates timed-out analyzer processes before release, and removes audio after persistence, terminal failure, retry release, or periodic lease-expiry recovery. Recovery continues after admission is disabled so pre-existing assets cannot be stranded by a feature-flag change.
- Online hot-set analysis prioritizes likes, playlist contents, completed and repeated plays, active Wave signals, and current top candidates across recently active accounts. YouTube identities are enriched through matched TIDAL ISRCs, existing TIDAL ISRCs resolve directly through MusicBrainz, and analyzer fingerprints can resolve or merge canonical recordings through a bounded AcoustID backfill without retaining provider audio.
- Similar Tracks has an authenticated online-first fallback endpoint that remains available without local embeddings or the optional Discover subsystem and becomes acoustically personalized when canonical DCLAP features exist.
...
- Authenticated accounts that still need taste setup receive an accessible Russian two-step genre and artist picker on the protected app shell; the editor uses account-keyed cache state, supports explicit skip and later replacement, and explains that its provider seeds do not create synthetic likes.
...
- The active Soundspan experience now uses the Spectral Stage design system across the shell, compact and full players, Home, Vibe, Search, catalog details, Library, authentication, onboarding, Settings, and secondary music workflows. Its artwork-led near-black stage, restrained violet/amber spectrum, readable semantic surfaces, shared success/warning/error states, consistent Lucide actions, hidden native carousel scrollbars, touch-safe controls, reduced-motion support, and responsive layouts cover 320 px phones through short and wide desktops.
- The remaining user-facing shell, player and queue, Search/import, authentication/settings, offline/PWA, device-download, Artist, Album, Mix, and initial-server states now use Russian copy for labels, loading and empty states, confirmations, accessible action names, and safe errors while retaining provider names, protocols, and catalog metadata.
...
- Device downloads prefer ordinary files below a user-selected folder on supported desktop Chromium browsers and app-private OPFS on Android Chromium so audio is not indexed as gallery media and folder permission is not requested after routine launches; browsers without either writable route remain unsupported. Private-storage downloads expose an explicit per-track Save as file action that hands a separate ordinary audio file to the browser/OS without removing the verified offline copy. Older public-folder copies remain addressable and expose a separate reconnect action when Android needs that folder permission again. A rejected durable-storage request is reported instead of being recorded as granted. IndexedDB keeps only owner-scoped metadata, queue state, and any persisted legacy directory handle, and existing verified CacheStorage copies migrate file-first after storage setup.
...
- Library, artist, album, ordinary-playlist, and My Liked surfaces now share one artwork-led detail language: long-title-safe heroes, predictable action docks, track-first content, touch-sized row controls, and bottom clearance for the persistent player. Library combines liked tracks, user playlists, and current-device downloads in one primary Playlists view, retains separate Albums and Artists views, and marks only verified downloaded track rows with a quiet device icon. The Playlists page exposes an explicit create action, including a sidebar deep link, while add-to-playlist uses a responsive portaled dialog that stays within the viewport.
- The authenticated shell uses an open edge-to-edge music canvas with a fixed 248 px desktop sidebar, Home/Wave/My Music destinations, compact Liked songs and Playlists shortcuts, a persistent global search bar, and an account drawer focused on device downloads, imports, notifications, and settings. Search results remain a deep-linkable canvas behind that global input instead of appearing as a duplicate navigation destination. The 128 px artwork-driven desktop dock keeps identity, a two-level transport, progress, and listening actions in three predictable regions while moving stream diagnostics into track overflow; mobile keeps menu plus search in the top bar, three destinations below, and a compact mini player. Player and overlay chrome share the same measured CSS boundaries and adapt to reduced motion, safe areas, and 320 px screens.
- Home now follows one editorial dashboard composition: an artwork-driven My Wave hero, four ordered Continue listening cards, a distinct Recently listened column, and five initially visible personal mixes. `Показать все` expands the remaining personal mixes in place instead of opening unrelated playlists, and the entire mixes shelf uses one opaque surface so the hero atmosphere cannot end halfway through its cards. The sections reuse one deduplicated playable queue, while the hero atmosphere blends into the page without covering content and the current artwork also drives the desktop player dock.
- Home and Explore are one online-first landing page: the legacy `/explore` entry redirects to Home, provider content is folded into one explicitly personal Stations for you row and one New & noteworthy row, empty rows stay hidden, and generic browse mixes or regional spillover cannot masquerade as personal stations. Direction and mood now live only in Vibe's tuning sheet instead of being duplicated as a Home link grid.
- Music players expose adjacent like and dislike controls with accessible pressed states and touch-sized targets; toggling an active dislike clears it, while playlist rows keep their compact like-only action.
- Search discovery merges public YouTube Music catalog songs into Last.fm results, preserving exact provider identities so those rows play immediately without a second fuzzy-match request. Music search uses All, Tracks, Artists, and Albums views: All immediately shows the canonical artist, five popular playable tracks, and a deduplicated album shelf without a Show all gate, while Tracks starts with a fast bounded page and progressively loads further merged matches instead of presenting fifty items as the complete catalog. Exact local-shadow duplicates resolve to one canonical provider artist or album card so full online profiles and releases remain reachable. Clicking a search row snapshots visible playable matches in screen order; obvious long-form videos rank after songs, and Soulseek appears only as a fallback when no playable song is available.
- Global search also preserves browsable YouTube Music album and artist identities: provider-only artists now open a catalog-backed page with playable popular tracks and online albums instead of treating a channel ID as a local artist name.
- YouTube Music and TIDAL thumbs-down preferences persist per user and are removed by a later like or clear action, so disliked remote tracks stay out of personalized home recommendations and future My Wave continuation pages; both ratings refresh the personal feed.
- Personalized YouTube Music discovery uses deterministic For you, New, and Familiar policies that combine likes, dislikes, completed listens, early skips, repeats, playlists, provider order, and artist diversity; artists known only from a skip or provider failure are not promoted as Familiar, and the base Wave remains independent from optional ListenBrainz connectivity.
- Vibe is now the online-first My Wave surface on every installation: one dominant Play/Pause action starts endless provider continuation without showing a misleading finite queue count, while a compact tuning sheet combines the server-backed For you, New to me, and Familiar directions with Calm, Energetic, Focus, Workout, Favorites, or Forgotten listening contexts. Applied direction and mood restore per account on the device after navigation or PWA restart, URL deep links override that restored view without overwriting it, and another account never inherits the selection. Starting My Wave explicitly disables shuffle so the ranked order is preserved; the default For you queue now treats recent listens as a sparse accent after fresh Quick Picks and discovery results instead of repeating roughly every third track, while Familiar remains intentionally history-led. The artwork-led ambient stage keeps the active choice, an honest next-pick preview, and current-track like, dislike, and skip feedback together; changing an active Wave immediately starts the first track from the newly ranked queue after a successful non-empty response, while unchanged settings do not restart playback and empty or failed retunes leave the existing queue intact. The page no longer mounts or advertises the legacy local Audio-DNA map, and its controls remain usable on phones without fighting the persistent mini player.
- URL playlist imports start as durable background jobs by default, while the slower per-track preview remains optional; settings distinguish Spotify metadata import, active YouTube Music playback, device-offline copies, and optional permanent server downloads.
- The streamlined Admin page keeps one explicit, default-off server-library safety control so administrators can unlock permanent local-album deletion without restoring dormant acquisition and maintenance panels.
...
- Split backend images now detect OpenSSL during dependency installation and embed the Debian 12 Prisma schema engine, so startup migrations do not depend on a runtime download from Prisma's CDN.
- Split workers now resolve TIDAL and YouTube Music sidecars through Compose DNS instead of incorrectly falling back to their own loopback interface.
- Single-host provider sidecars now retain the isolated proxy egress network and receive standard proxy variables, preventing YouTube Music DNS failures after an AIO-to-split migration.
- YouTube Music now prepares the next queue item after one second of confirmed audible playback instead of waiting for the last twenty seconds. The single bounded preload makes ordinary manual skips and natural transitions reuse the sidecar spool while avoiding provider work before the selected track has actually started.
- Browser cancellation now aborts pending YouTube Music proxy acquisition before a superseded cold spool can occupy provider capacity; completed response closes remain normal and do not count as provider failures.
- Playback telemetry separates superseded client cancellation, provider cooldown skips, rate limits, timeouts, network failures, unavailable media, provider challenges, and unsupported sources instead of collapsing every failure into one opaque error.
- A server-confirmed unavailable provider identity is suppressed for ten minutes during automatic queue cycling, while a deliberate manual selection can still retry it immediately; distinct unavailable tracks continue advancing under the existing bounded recovery breaker.

- Opening the player's Similar tab accepts both the current string artist contract and legacy object-shaped responses instead of crashing the React tree to a black screen. The backend now emits the documented string artist name for new responses.
- Rapid manual YouTube Music track selections are coalesced before opening a remote stream, so superseded selections and their reactive next-track preloads do not fill the sidecar's bounded spool queue and starve the track the listener actually chose. Initial playback, automatic queue advance, verified offline copies, and the single natural-end preload used to retain the iOS audio session remain immediate; late preload completions cannot replace a newer source.
- Metadata-only catalog tracks without a local file path are excluded from audio-analysis admission, progress totals, database reconciliation, and stale Redis claims, so they cannot accumulate misleading `Invalid audio path` failures.
- Incremental Audiobookshelf synchronization prunes books only after verified complete listings and a two-snapshot absence grace, preserves deletion tombstones, and relies on database foreign keys to cascade progress or clear playback references safely. Concurrent progress writes for an already-removed book return not found, while playback-state writes retry once without the stale audiobook reference.
- Canonical remote-analysis assets now decode through a bounded system-FFmpeg PCM boundary before Essentia, so Opus/WebM provider streams remain analyzable despite their neutral `.audio` spool name and decoder failures become explicit terminal results with owned-file cleanup instead of crashing the process pool or recording an empty completion.
...
- Search album cards preserve the canonical release-group identity of local catalog shadows, so an empty local record falls through to its playable online album instead of opening a broken internal-ID route.
...
- Desktop Activity now opens as an on-demand overlay instead of reserving a persistent right rail or restoring itself across visits; ordinary accounts see personal notifications and imports without library scan, enrichment, or social-presence noise.
- Installed iOS PWAs keep native playback on the browser-owned media element instead of routing it through a background-suspended Web Audio context, so minimizing the app no longer leaves a moving lock-screen timer with silent output or a repeated sample when pausing. OS media controls attach on the first local play, remain stable while position updates advance, and release their handlers, metadata, playback state, and scrubber position after the active item is cleared.
- Unverified legacy device-download metadata no longer shadows a healthy online stream with a missing or truncated CacheStorage URL; only a verified prepared Blob or managed device file can replace the network source.
- Global YouTube Music search no longer turns artist channels or ordinary playlists into broken album rows, resolves TV album-playlist identities through the provider's canonical album lookup when opened, retains an exact provider artist identity when a same-name local artist exists, and invalidates cached discovery rows created by the older mapping. Public native-search failures now fall back only for the affected request instead of permanently downgrading every user to TV search.
- Device-storage read failures keep the last verified download snapshot visible and offer an explicit retry in Downloads and Settings; offline-copy deletion confirms that only the current device is affected, and both desktop and mobile Library navigation open the precached Downloads document while disconnected. Mobile fixed bars own safe-area insets without double padding, compact controls meet the 44 px touch target, and recommendation and Library failures expose retry actions.
- Muted semantic labels now use an AA-safe contrast token, while legacy provider-green toggle and Vibe accents follow the shared Soundspan brand palette.
- Unavailable or age-restricted YouTube Music identities recover to another exact, stream-validated catalog candidate without skipping the queue item; permanent provider reports for unavailable, private, removed, deleted, or terminated-account videos cross the sidecar boundary as missing content, while transient extraction and network failures remain retryable server errors. Stale and duplicate recovery requests are correlated and bounded, the exact recovery proxy path receives enough time to finish, and owned playlist rows are updated with a safe compare-and-swap when possible.
- Public YouTube Music search recognizes the current TVHTML5 lockup response shape, including playable IDs, artist and album metadata, artwork, duration, and duplicate suppression.
...
- Device downloads use renewable, attempt-aware browser-storage leases and a bounded background-completion grace, so concurrent tabs, logout, account changes, retries, deletes, and stale foreground or legacy Android Background Fetch completions cannot publish another session's state or resurrect a removed copy. Owner-bound authentication leases now abort in-flight credentialed transfers synchronously and guard IndexedDB and CacheStorage publication, including a logout followed by login to the same account. Ready state requires a retained, non-empty cache body whose measured bytes match the declared size; Downloads playback uses that local CacheStorage Blob directly, and offline failures pause once instead of cascading through the queue. Service-worker updates keep the previous offline shell active unless both Downloads documents and all discovered Next.js runtime chunks are cached successfully.
- Device-download metadata changes refresh other open tabs through an opaque signal, and a transient IndexedDB open failure can recover without reloading the app. Automatic liked-song cleanup cannot remove a copy that was promoted to a manual download, while a like arriving during an active synchronization schedules one fresh snapshot instead of being lost.
- A stale persisted playback flag can no longer postpone a waiting service worker forever: activation now requires both a live playback state and a fresh in-memory engine heartbeat, while the worker keeps its existing single client navigation after a safe shell update.
- Foreground device downloads report throttled measured bytes and percentage when size is known, while collection queues expose queued, active, interrupted, and failed work without presenting it as playable before CacheStorage verification.
...
- Imported and remote YouTube Music tracks with a valid provider ID now receive a deterministic provider thumbnail when their stored thumbnail is missing, across playlist, liked-library, and play-history response shapes, without synthesizing URLs from malformed IDs.
- Vibe no longer gates recommendations on local files or analysis status; My Wave remains available with zero server-library tracks and explains the account signals that improve it.
- All-in-one images preserve the Prisma client generated from the current schema after adopting the pinned release dependency tree, and the image build verifies the remote-preference, saved-music, and playback-engagement schema before publishing, so the online-first flows cannot start with a stale client.
- Background playlist imports reject preview/submit races, atomically deduplicate concurrent submissions, never move a completed job back to cancelling, retain custom or provider playlist names in Activity, refresh an already-open Imports tab, serialize event refreshes behind slow status polls, and invalidate cached personalized shelves when background or preview imports complete, even after leaving the Import page.
- Retried background playlist imports reuse one owner-scoped playlist and its validated resolution snapshot, preserve later user item removals instead of repopulating an already-created playlist, recover safely from concurrent unique-key races, and reconcile a committed playlist before terminal failure or an immediate failed-job resubmit can create a duplicate; later deliberate reimports remain available. A late cancellation that still commits a playlist keeps the playlist link and warning visible in Activity and refreshes playlist and personalized-home caches.
- Spotify playlist imports preserve repeated track occurrences in source order, fail closed if playlist-item persistence is incomplete, and leave transient provider timeouts or server errors to the bounded queue retry policy without retry-looping rate limits or deterministic completeness failures. Credential-free imports use Spotify's anonymous embed session to paginate the complete public playlist and refuse an unverifiable 100-item embed truncation instead of silently producing a partial playlist.
- Cold YouTube Music HLS spools fetch a bounded set of fragments concurrently and suppress per-fragment console progress, reducing the wait before the complete local file is served while retaining ordinary byte-range seeking and avoiding expired signed-URL continuation failures. New Android device downloads use the verified foreground path instead of Chromium Background Fetch, while legacy stuck registrations are aborted into a safe retry rather than remaining at 0% indefinitely; a redundant Android registration is also cleared when the verified device copy is already ready.
- Playback errors honor the engine's terminal recovery decision, reset retry state only after confirmed audible progress, skip failed tracks even under repeat-one, and continue multi-track queues after terminal load failures without repeatedly retrying one unavailable provider item.
- Quick Picks and other remote YouTube Music queues append fresh personalized provider tracks before the final item ends, carry bounded queue exclusions, rotate across later taste seeds, preserve the active Wave mode, and skip a repeated provider page before giving up; they preserve completed shuffle order and advance only after both the matching provider request succeeds and the updated queue commits. Stale completions cannot advance a manually selected duplicate queue entry or outlive the player, and local-library queues continue to use Audio-DNA similarity.
- Provider-radio continuation uses the same bounded 17-second request budget as personalized Home, allowing the backend's 13-second YouTube Music radio window to complete instead of aborting valid responses early.
- Partial external-search responses are no longer cached for fifteen minutes when any catalog source fails, cache identity includes the live Last.fm configuration state, and YouTube Music search runs in a bounded three-slot sidecar pool with a six-second deadline instead of accumulating unbounded worker threads. Discovery fetches tracks, albums, and artists through one queued sidecar batch per request, so one user cannot consume every fail-fast search slot and make a concurrent user's catalog disappear; cancelled batches retain their slots until blocking provider threads actually settle. Remote likes reuse artwork already present on the track instead of blocking on another metadata lookup.
- Provider thumbnails use the deployment's trusted Node egress proxy, optional browse artwork fails within a bounded window, and shared player/list artwork hides a failed image instead of leaving a broken-image glyph on screen.
- Primary desktop, TV, and search navigation stays music-first by hiding the unused podcast and unconfigured audiobook destinations; their dormant backend APIs remain available for a future licensed catalog integration.
