...
- The active Soundspan experience now uses the Spectral Stage design system across the shell, compact and full players, Home, Vibe, Search, catalog details, Library, authentication, onboarding, Settings, and secondary music workflows. The new artwork-led hierarchy replaces the near-black framed workspace with a layered cool-neutral canvas, readable semantic surfaces, a restrained periwinkle action color, shared success/warning/error states, consistent Lucide actions, hidden native carousel scrollbars, touch-safe controls, reduced-motion support, and responsive layouts from 320 px phones through desktop.
...
- The authenticated shell uses an open edge-to-edge music canvas instead of a framed workspace, with compact Home, Vibe, and Library navigation, a persistent global search bar, a quiet 224 px playlist rail, and an account drawer focused on liked songs, playlists, device downloads, imports, notifications, and settings. Search results remain a deep-linkable canvas behind that global input instead of appearing as a duplicate navigation destination. The 80 px desktop player keeps identity, primary transport, and listening actions in three predictable regions while moving stream diagnostics into track overflow; mobile keeps menu plus search in the top bar, three destinations below, and a 64 px mini player focused on artwork, title, like, and play. Player chrome adapts to reduced motion, reduced transparency, high contrast, safe areas, and 320 px screens.
- Home opens with a compact one-tap My Wave action using artwork from the active account feed, puts Continue listening ahead of discovery, and exposes a bounded Made For You set of distinct, playable account-backed or generated mixes whose visible tracks do not repeat across cards.
