...
- The authenticated shell uses one restrained music stage with compact four-destination navigation, a wider desktop search bar, a quieter playlist sidebar, and an account drawer focused on liked songs, playlists, device downloads, imports, and settings. The artwork-aware desktop player and full-screen mobile player keep the primary transport centered, show preference controls predictably, reserve seek-by-seconds actions for long-form audio, and adapt to reduced motion, reduced transparency, high contrast, safe areas, and 320 px screens.
