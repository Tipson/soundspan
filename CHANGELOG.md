...
- The authenticated shell uses one restrained music stage with a compact four-destination navigation model, a wider desktop search bar, a quieter playlist sidebar, and an account drawer focused on liked songs, playlists, device downloads, imports, and settings. Mobile topbar, mini-player, and bottom navigation own separate safe-area zones, preserve 44 px controls, and keep useful title space at 320 px without duplicating Home or Back controls.
- Home opens with an immersive one-tap My Wave launch using artwork from the active account feed, compact shortcuts to everyday music actions, and a Made For You rail that combines only non-empty Quick picks, Fresh finds, Listen again, Discover Weekly, and generated mixes. Two focused track rails replace the former repeated shelf dump, and mobile bottom spacing keeps them clear of the fixed player.
- Home and Explore are one online-first landing page: the legacy `/explore` entry redirects to Home, live YouTube Music mixes, categories, shelves, and charts follow the personal feed, empty generated playlists stay hidden, and the primary navigation is reduced to Home, Search, Vibe, and Library without starting the optional Listen Together polling loop. The catalog portion is curated to three unique navigable shelves with bounded item and category previews, suppressing irrelevant regional spillover instead of rendering every provider row.
...
- Search discovery merges public YouTube Music catalog songs into Last.fm results, preserving exact provider identities so those rows play immediately without a second fuzzy-match request. Music search uses All, Tracks, Artists, and Albums views: All keeps an exact-aware top result beside a five-track continuation with an explicit Show all action and one six-album shelf, while Tracks caps the merged visible result set at 50. Exact local-shadow duplicates now resolve to one canonical provider artist or album card so full online profiles and releases remain reachable. Clicking a search row snapshots only visible playable matches in screen order; obvious long-form videos rank after songs, and Soulseek appears only as a fallback when no playable song is available.
...
- Vibe is now the online-first My Wave surface on every installation: an accessible Tune sheet offers only the server-backed For you, New to me, and Familiar directions, starting a Wave enables endless provider continuation, and the page no longer mounts or advertises the legacy local Audio-DNA map. Its immersive listening-room layout centers one signal-dial launch action, explains continuous replenishment without a finite queue count, and keeps the active direction plus current-track feedback in context; the Tune sheet shows the real tradeoff between personal balance, discovery, and familiar rotation. Retuning an active Wave keeps the current song playing while immediately replacing every upcoming item with the newly ranked direction. On phones and tablets, the layout wraps long direction copy and leaves the persistent mini player as the single current-track control surface instead of duplicating or covering controls inside Vibe.
...
- Installed iOS PWAs keep native playback on the browser-owned media element instead of routing it through a background-suspended Web Audio context, so minimizing the app no longer leaves a moving lock-screen timer with silent output or a repeated sample when pausing.
