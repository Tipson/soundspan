...
- Search discovery merges public YouTube Music catalog songs into Last.fm results, preserving exact provider identities so those rows play immediately without a second fuzzy-match request. Music search uses All, Tracks, Artists, and Albums views: All immediately shows the canonical artist, five popular playable tracks, and a deduplicated album shelf without a Show all gate, while Tracks exposes up to 50 merged visible results. Exact local-shadow duplicates resolve to one canonical provider artist or album card so full online profiles and releases remain reachable. Clicking a search row snapshots visible playable matches in screen order; obvious long-form videos rank after songs, and Soulseek appears only as a fallback when no playable song is available.
