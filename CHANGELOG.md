...
- Vibe is now the online-first My Wave surface on every installation: an accessible Tune sheet offers only the server-backed For you, New to me, and Familiar directions, starting a Wave enables endless provider continuation, and the page no longer mounts or advertises the legacy local Audio-DNA map. Its focused spectral radio layout keeps one primary launch action and current-track feedback while removing the finite seed-count label and horizontal preview shelf that made the endless flow look like a fixed playlist. On phones and tablets, the persistent mini player is the single current-track control surface instead of duplicating the same controls inside Vibe.
