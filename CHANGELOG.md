...
- Vibe is now the online-first My Wave surface on every installation: one dominant Play/Pause action starts endless provider continuation without showing a misleading finite queue count, while a compact tuning sheet combines the server-backed For you, New to me, and Familiar directions with Calm, Energetic, Focus, Workout, Favorites, or Forgotten listening contexts. Starting My Wave explicitly disables shuffle so the ranked order is preserved. The artwork-led ambient stage keeps the active choice and current-track like, dislike, and skip feedback together; retuning an active Wave from Home or the tuning sheet preserves the current song and replaces upcoming items only after a successful non-empty recommendation response, leaving the existing queue intact on empty or failed retunes. The page no longer mounts or advertises the legacy local Audio-DNA map, and its controls remain usable on phones without fighting the persistent mini player.
