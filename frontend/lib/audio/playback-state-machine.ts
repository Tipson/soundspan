...
export class PlaybackStateMachine {
    ...
    constructor() {
        ...
        if (typeof window !== "undefined") {
            this.debugEnabled =
                localStorage.getItem("soundspanAudioDebug") === "1";
        }
    }
    ...
    transition(
        to: PlaybackState,
        options?: { error?: string; errorCode?: number },
    ): boolean {
        ...
        const error =
            to === "ERROR" ? (options?.error ?? "Unknown error") : null;
    ...
    get isLoading(): boolean {
        return this.context.state === "LOADING";
    }
    get isRecovering(): boolean {
        return this.context.state === "RECOVERING";
    }
    get isReady(): boolean {
    ...
    get isSeeking(): boolean {
        return this.context.state === "SEEKING";
    }
    get isBuffering(): boolean {
        return this.context.state === "BUFFERING";
    }
    ...
    get canPlay(): boolean {
        return (
            this.context.state === "READY" || this.context.state === "PLAYING"
        );
    }
}
