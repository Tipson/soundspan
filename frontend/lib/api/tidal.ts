...
export function WithTidal<TBase extends ApiClientConstructor>(Base: TBase) {
    abstract class TidalApi extends Base {
        ...
        async pollTidalAuth(deviceCode: string): Promise<{
            ...
            return this.post(`/tidal-streaming/auth/device-code/poll`, {
                deviceCode,
            });
        }
        ...
        async getTidalGenrePlaylists(path: string): Promise<{
            ...
            return this.get(
                `/browse/tidal/genre-playlists?path=${encodeURIComponent(path)}`,
            );
        }

        async getTidalBrowsePlaylist(
            id: string,
            limit?: number,
        ): Promise<{
            id: string;
            ...
            const qs = params.toString();
            return this.get(
                `/browse/tidal/playlist/${encodeURIComponent(id)}${qs ? `?${qs}` : ""}`,
            );
        }
