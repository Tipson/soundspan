...
export function classifyGesture(dx, dy, velocity = 0) {
    ...
    if (Math.abs(dy) > Math.abs(dx) * 0.8) return "cancel";
    if (Math.abs(dx) > 55 || (Math.abs(dx) > 20 && Math.abs(velocity) > 0.45))
...
export function createPlayer(engine, onChange, count = 3) {
    ...
    async function play() {
        ...
        try {
            ...
            state.playing = false;
            state.status = "error";
            state.error = "Не удалось включить звук. Нажми ещё раз.";
            emit();
        }
    }
    function pause() {
    ...
    function step(delta) {
        state.index = (((state.index + delta) % count) + count) % count;
        engine.stop();
