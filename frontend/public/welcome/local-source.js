...
export function createLocalSource(records) {
    ...
    return async (index, events) => {
        ...
        if (!Audio)
            throw new Error("Этот браузер не поддерживает воспроизведение.");
        context ??= new Audio();
        const resumed = context.resume(),
            path = records[index].audio;
        if (!buffers.has(path)) {
            const pending = fetch(path, { signal: AbortSignal.timeout(12000) })
                .then(async (response) => {
                    if (!response.ok)
                        throw new Error("Эта запись пока недоступна.");
                ...
                .catch((error) => {
                    if (error.name === "TimeoutError")
                        throw new Error(
                            "Загрузка записи задерживается. Попробуй ещё раз.",
                        );
                    if (error instanceof TypeError)
                        throw new Error(
                            "Нет связи с записью. Попробуй ещё раз.",
                        );
        ...
        function stop() {
            ...
            old.gain.gain.linearRampToValueAtTime(0, now + 0.07);
            old.source.stop(now + 0.08);
            old.source.onended = () => {
        ...
        return {
            play() {
                ...
                voice = { source, gain };
                gain.gain.linearRampToValueAtTime(0.65, started + 0.14);
