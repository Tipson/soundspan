...
export function deriveVibeMapPresentation(input: VibeMapPresentationInput) {
    const effectiveHighlightIds =
        input.sweepHighlight ?? input.vibeHighlight ?? input.spotlightHighlight;
    return {
        effectiveHighlightIds,
        effectiveDim: input.sweepHighlight
            ? false
...
export function findTrackAtScreenPoint(
    ...
    const radius =
        Math.max(
            computeDotRadius(input.viewport.scale, input.fitScale),
            MIN_HIT_RADIUS,
        ) * (input.radiusScale ?? 1);
    const available = Math.min(
        input.tracks.length,
        input.mask.length,
        Math.floor(input.positions.length / 2),
        MAX_HIT_TEST_TRACKS,
    );
    ...
    for (
        let index = 0;
        index < MAX_HIT_TEST_TRACKS && index < available;
        index++
    ) {
        if (input.mask[index] === 0) continue;
        const dx =
            x -
            (input.positions[index * 2] * input.viewport.scale +
                input.viewport.tx);
        const dy =
            y -
            (input.positions[index * 2 + 1] * input.viewport.scale +
                input.viewport.ty);
...
export function journeyBounds(
    originId: string | null,
    waypoints: readonly JourneyPoint[],
    ...
    for (
        let index = 0;
        index < MAX_JOURNEY_WAYPOINTS && index < waypoints.length;
        index++
    ) {
...
export function shouldFollowTravelOrigin(
    screen: Point,
    dims: MapDims,
): boolean {
    ...
    return (
        screen.x < marginX ||
        screen.x > dims.width - marginX ||
        screen.y < marginY ||
        screen.y > dims.height - marginY
    );
}
