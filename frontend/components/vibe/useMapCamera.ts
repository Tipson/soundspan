...
function applyCameraFrame(
    ...
    if (pan.dx !== 0 || pan.dy !== 0) {
        next = clampViewport(
            { scale: next.scale, tx: next.tx + pan.dx, ty: next.ty + pan.dy },
            dims,
        );
...
function useViewportState(
    ...
    Viewport | null,
    Dispatch<SetStateAction<Viewport | null>>,
    React.MutableRefObject<Viewport | null>,
] {
...
function useCameraEngine(
    ...
    const tick = useCallback(function cameraTick(now: number) {
        ...
        if (result.continueFlight)
            refs.raf.current = requestAnimationFrame(cameraTick);
