...
type SimilarTrack = Awaited<
    ReturnType<typeof api.getVibeSimilarTracks>
>["tracks"][number];
...
function toCompassCandidate(track: SimilarTrack): CompassCandidate {
    return {
        ...
        album: track.album,
        artist: track.artist,
        similarity: Math.max(0, 1 - track.distance / 2),
        ...
        arousal: track.audioFeatures?.arousal ?? null,
        moods: null,
    };
}
...
function originFeatures(
    ...
    const source =
        neighbors?.originId === currentId ? neighbors.sourceFeatures : null;
    return {
        energy: origin.energy,
        valence: origin.valence,
        danceability: source?.danceability ?? null,
        arousal: source?.arousal ?? null,
    };
}

function useTravelDerivations(
    args: UseTravelModeArgs,
    currentId: string | null,
    neighbors: NeighborState | null,
) {
    const enriched = useMemo(
        () =>
            neighbors?.originId === currentId
    ...
    const onMap = useMemo(
        () => shown.filter((item) => args.trackById.has(item.id)),
    ...
    const byId = useMemo(
        () => new Map(shown.map((item) => [item.id, item])),
        [shown],
    );
    return {
        onMap,
...
function useTravelActions(
    ...
    const navigate = useCallback(
        (id: string) => {
            ...
            if (args.trackById.has(id))
                args.dispatch({ type: "TRAVEL_TO", id });
        },
        [byId, args],
    );
...
export function useTravelMode(args: UseTravelModeArgs): TravelView | null {
    const currentId =
        args.state.mode === "travel" ? args.state.currentId : null;
    ...
    return {
        ...
        breadcrumbTitles: args.state.breadcrumb.map((id) => ({
            id,
            title: args.titleOf(id),
        })),
        direction: args.state.direction,
        onMapNeighbors: derived.onMap,
        offMapNeighbors: derived.offMap,
        loading: request.loading,
        error: request.error,
        quantiles: args.quantiles,
        originFeatures: derived.features,
        setDirection: (direction) =>
            args.dispatch({ type: "SET_DIRECTION", direction }),
        navigate: actions.navigate,
        queue: actions.queue,
        close: args.exitToExplore,
    };
}
