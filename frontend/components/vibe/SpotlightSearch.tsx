        ...
        async (event?: FormEvent) => {
            ...
            try {
                ...
                if (token === tokenRef.current) {
                    onResults(
                        new Set(response.tracks.map((track) => track.id)),
                        value,
                    );
                }
            } catch {
...
function useDropdownRows(
    ...
    return useMemo(() => {
        ...
        return [
            ...matches.map((track): DropdownRow => ({ kind: "match", track })),
            { kind: "vibe" },
        ];
    }, [tracks, query]);
}
...
function useDropdownController(args: {
    ...
    const keyDown = useCallback(
        (event: KeyboardEvent<HTMLInputElement>) => {
            if (
                show &&
                handleNavigationKey(event, args.rows, activeIndex, setActive)
            )
                return;
        ...
        <li id={optionId} role="option" aria-selected={active}>
            ...
            >
                <span
                    ...
                    style={{
                        backgroundColor: getMoodColor(row.track.dominantMood),
                    }}
                    aria-hidden="true"
                />
...
function SearchDropdown({
    ...
    return (
        ...
        >
            {rows.map((row, index) => (
                <DropdownOption
                    key={row.kind === "vibe" ? "vibe" : row.track.id}
                    row={row}
                    index={index}
...
function SearchForm({
    ...
    return (
        ...
        >
            <div className="pointer-events-auto relative flex items-center w-[min(80vw,320px)] h-10 rounded-full bg-black/60 backdrop-blur-md border border-white/10 shadow-lg focus-within:border-white/20 transition-colors">
                ...
                <input
                    ...
                    aria-activedescendant={
                        controller.show
                            ? `spotlight-option-${controller.activeIndex}`
                            : undefined
                    }
            ...
            {remote.error && (
                <p className="mt-1 px-3 text-xs text-red-400">{remote.error}</p>
            )}
...
export function SpotlightSearch(props: SpotlightSearchProps) {
    ...
    const controller = useDropdownController({
        ...
        collapse: () => isSmall && setExpanded(false),
    });
    if (isSmall && !expanded) {
        return (
            <CollapsedSearch
