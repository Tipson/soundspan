...
import { createDebouncedStorageFlush } from "../../lib/playback-state-cadence";
...
test("schedule fires after specified delay", async () => {
    ...
    assert.ok(
        elapsed >= 55,
        `should fire after ~60ms, fired after ${elapsed}ms`,
    );
});
...
test("flush immediately runs pending callback", () => {
    ...
    assert.equal(
        flushCount,
        1,
        "flush should fire the pending callback immediately",
    );
});
