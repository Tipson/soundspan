...
test("job failure message returns a non-empty string error verbatim", () => {
    assert.equal(
        resolveJobFailureMessage({ error: "Scan failed" }),
        "Scan failed",
    );
});
...
test("job failure message falls back for an undefined result", () => {
    assert.equal(
        resolveJobFailureMessage(undefined),
...
test("job failure message falls back for non-string errors", () => {
    assert.equal(
        resolveJobFailureMessage({ error: 42 }),
        "Job failed with unknown error",
    );
    assert.equal(
        resolveJobFailureMessage({ error: { message: "Failed" } }),
        "Job failed with unknown error",
    );
});
...
test("job failure message falls back for an empty string error", () => {
    assert.equal(
        resolveJobFailureMessage({ error: "" }),
        "Job failed with unknown error",
    );
});
