...
describe("unified recommendation engine", () => {
    ...
    it.each(["baseline", "active", "shadow"] as const)(
        "starts Wave history while candidates are pending in %s",
        async (mode) => {
            const deps = dependencies(mode);
            let release!: () => void;
            deps.loadCandidates.mockImplementation(
                () =>
                    new Promise((resolve) => {
                        release = () =>
                            resolve({
            ...
            const pending = new RecommendationEngine(deps).recommend(request);
            try {
                await Promise.resolve();
                expect(deps.loadRecentExposures).toHaveBeenCalledTimes(1);
                expect(deps.loadDislikedCanonicalKeys).toHaveBeenCalledTimes(1);
                expect(deps.loadTasteContext).toHaveBeenCalledTimes(
                    mode === "baseline" ? 0 : 1,
                );
            ...
            expect(deps.loadRecentExposures).toHaveBeenCalledTimes(1);
            expect(
                deps.recordGeneration.mock.calls[0][0].recommendations.map(
                    (row: { track: RecommendationCandidate }) => row.track.id,
                ),
            ).toEqual(["yt:fresh"]);
        },
    );

    it("keeps baseline Home free of history and taste reads", async () => {
        ...
        expect(deps.loadDislikedCanonicalKeys).not.toHaveBeenCalled();
        expect(deps.loadTasteContext).not.toHaveBeenCalled();
    });

    it("settles optional failures after an early candidate failure without recording history", async () => {
        const deps = dependencies();
        const error = new Error("candidate failure");
        let rejectTaste!: (error: Error) => void;
        deps.loadTasteContext.mockImplementation(
            () =>
                new Promise((_, reject) => {
                    rejectTaste = reject;
                }),
        );
        deps.loadCandidates.mockRejectedValue(error);
        await expect(
            new RecommendationEngine(deps).recommend(request),
        ).rejects.toBe(error);
        expect(deps.loadTasteContext).toHaveBeenCalledTimes(1);
        rejectTaste(new Error("late optional failure"));
