...
describe("DiscoveryBatchLogger", () => {
    ...
    describe("addLog", () => {
        it("should append log entry to existing logs", async () => {
            ...
            (
                mockPrisma.discoveryBatch.findUnique as jest.Mock
            ).mockResolvedValue({
                id: "batch-123",
                logs: existingLogs,
            });
        ...
        it("should trim logs to 100 entries max", async () => {
            const existingLogs: BatchLogEntry[] = Array.from(
                { length: 100 },
                (_, i) => ({
            ...
            const updateCall = (mockPrisma.discoveryBatch.update as jest.Mock)
                .mock.calls[0][0];
        ...
        it("should not throw when database operation fails", async () => {
            ...
            const logger = new DiscoveryBatchLogger();
            await expect(
                logger.addLog("batch-123", "error", "Test"),
