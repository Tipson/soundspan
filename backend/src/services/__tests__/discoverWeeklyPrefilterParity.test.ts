...
describe("discover weekly artist library membership prefilter parity", () => {
    ...
    it("classifies a mixed candidate set identically via the batched prefetch and the old per-candidate DB path, issuing exactly one findMany call", async () => {
        ...
        const dbRows = [
            ...
            {
                mbid: "mbid-casefold-owner",
                name: "CaseFold Artist",
                albumCount: 1,
            },
            {
                mbid: "mbid-fallthrough-src",
                name: "Fallthrough Empty",
                albumCount: 0,
            },
            {
                mbid: "mbid-other-owner",
        ...
        (prisma.artist.findFirst as jest.Mock).mockImplementation(
            async (query: any) => {
        ...
        (prisma.artist.findMany as jest.Mock).mockImplementation(
            async (query: any) => {
                const orClauses: any[] = query?.where?.OR ?? [];
                const matched = dbRows.filter((row) =>
                    orClauses.some((clause) => {
                        if (clause.mbid?.in)
                            return clause.mbid.in.includes(row.mbid);
                        if (clause.name?.equals) {
                            return (
                                row.name.toLowerCase() ===
                                String(clause.name.equals).toLowerCase()
                            );
                        }
                        return false;
                    }),
        ...
        const candidates: Array<{
            ...
            },
            {
                ...
                mbid: undefined,
                expected: true,
                note: "name-case-insensitive hit",
            },
            {
                name: "Zero Album Owner",
                mbid: "mbid-zero-album",
            ...
            {
                name: "Nobody Home",
                mbid: "mbid-does-not-exist",
                expected: false,
                note: "no match by either probe",
            },
        ];
        ...
        for (const c of candidates) {
            newResults.push(
                await service.isArtistInLibrary(c.name, c.mbid, membership),
            );
        }
