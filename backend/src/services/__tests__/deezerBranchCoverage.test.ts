...
describe("deezer branch coverage", () => {
    ...
    it("parseUrl handles valid url shapes and rejects malformed/unsupported ids", () => {
        expect(
            deezerService.parseUrl("https://www.deezer.com/us/track/123456"),
        ).toEqual({
            type: "track",
            id: "123456",
        });
        expect(
            deezerService.parseUrl("https://www.deezer.com/album/987654?utm=1"),
        ).toEqual({
            type: "album",
            id: "987654",
        });
        expect(
            deezerService.parseUrl("https://www.deezer.com/playlist/321654/"),
        ).toEqual({
            type: "playlist",
            id: "321654",
        });
        ...
        expect(
            deezerService.parseUrl("https://www.deezer.com/artist/27"),
        ).toBeNull();
        expect(
            deezerService.parseUrl("https://www.deezer.com/track/not-a-number"),
        ).toBeNull();
        expect(
            deezerService.parseUrl("https://www.deezer.com/album/"),
        ).toBeNull();
    });
    ...
    it("album cover search handles query failures, null payloads, and image fallback fields", async () => {
        ...
        await expect(
            deezerService.getAlbumCover("Artist", "Alpha"),
        ).resolves.toBe("https://img/alpha-big.jpg");
    ...
    it("track album handles cached null, malformed cache, and null album branch", async () => {
        ...
        await expect(
            deezerService.getTrackAlbum("Artist", "Song"),
        ).resolves.toBeNull();
        expect(mockRedisSetEx).toHaveBeenCalledWith(
    ...
    it("uses cache and fallback image/preview fields for artist and track lookups", async () => {
        ...
        await expect(
            deezerService.getArtistImage("Cached Artist"),
        ).resolves.toBe("https://cached/artist.jpg");
        ...
        ).resolves.toBe("https://img/artist-medium.jpg");

        mockRedisGet.mockResolvedValueOnce("null");
        await expect(
            deezerService.getTrackPreview("A", "B"),
        ).resolves.toBeNull();

        mockRedisGet.mockResolvedValueOnce(null);
        mockAxiosGet.mockResolvedValueOnce({
            data: { data: [{ preview: "https://p/1.mp3" }] },
        });
        await expect(
            deezerService.getTrackPreview("Artist", "Track"),
        ).resolves.toBe("https://p/1.mp3");
    });
        ...
        mockAxiosGet.mockResolvedValueOnce({
            data: {
                data: [
                    {
                        name: "Ghostface",
                        picture_medium: "https://img/ghostface.jpg",
                    },
    ...
    it("covers featured/genres/radios/editorial aggregation branches", async () => {
        ...
        mockAxiosGet.mockResolvedValueOnce({
            data: {
                data: [
                    { id: 0, name: "All" },
                    { id: 2, name: "Rock", picture: "https://g.jpg" },
                ],
            },
        });
        ...
        ).resolves.toEqual([expect.objectContaining({ id: "gp-1" })]);
        ...
        mockAxiosGet
            .mockResolvedValueOnce({
                data: { title: "Drive", picture: "https://drive.jpg" },
            })
            .mockResolvedValueOnce({
                data: {
                    data: [
    ...
    it("returns [] for chart/search failures and maps empty-data results", async () => {
        ...
        await expect(
            deezerService.searchPlaylists("r&b / dance", 2),
        ).resolves.toEqual([]);
        expect(mockLoggerError).toHaveBeenCalledWith(
    ...
    it("covers fallback branches across preview/album/playlist/chart/search transforms", async () => {
        ...
        await expect(
            deezerService.getAlbumCover("Cached", "Album"),
        ).resolves.toBe("https://cached/cover.jpg");
        ...
        await expect(
            deezerService.getTrackPreview("No", "Preview"),
        ).resolves.toBeNull();
        ...
        await expect(
            deezerService.getTrackAlbum("Exact", "Song"),
        ).resolves.toEqual({
        ...
        await expect(
            deezerService.searchPlaylists("unicode ✓", 1),
        ).resolves.toEqual([
            {
                id: "12",
                title: "Unknown",
