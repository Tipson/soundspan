...
- **File-size advisory:** `scripts/ci/check-file-size.mjs` scans non-test production source under `backend/src`, the frontend `app`/`components`/`features`/`hooks`/`lib` trees, Python files under `services`, and each package's `src` tree. It reports files over 3,000 lines and growth beyond the frozen per-file baseline, but the report is informational and does not block a PR. Lower or remove baseline entries as files are split.
...
| Enforcement Gates                 | Enforcement | `npm run verify:gates`                                                                                          | route-error canonicalization + self-test, source-file-size checker self-test, environment-variable documentation/deployment parity ratchet + self-test, npm override staleness + self-test, Dockerfile hygiene, infrastructure/release helper tests, frontend hardcoded-hex baseline, OpenAPI route sync, and repo-wide Prettier format check; the actual source-file-size report is advisory in CI, while local `verify:gates` also runs the infrastructure/release helper tests that the CI enforcement job does not currently run |
