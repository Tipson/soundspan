...
| iOS source handoff   | `frontend/lib/audio-engine/iosBackgroundTrackHandoff.ts` | Schedules one pre-end queue advance only for a hidden installed iOS/iPadOS PWA after the next source has entered preload.                                                                                                                                                                            |
...
`iosStandalonePwaBridge.ts` retains the bridge implementation as an isolated, injectable compatibility seam, but its production policy gate is deliberately closed on every platform. When an installed iOS/iPadOS PWA is hidden and the next queue item has entered preload, the orchestrator schedules a one-shot handoff for approximately 250 ms before the current source ends. This keeps WebKit's media session active while the source changes; an end-adjacent `timeupdate` is the fallback when the timer is throttled. The handoff is cancelled on pause, seek, source change, teardown, Listen Together, repeat-one, a missing next item, or a next item whose preload has not started. Safari tabs and every non-iOS-standalone client retain ordinary native `ended` handling. No Web Audio keepalive is used.
