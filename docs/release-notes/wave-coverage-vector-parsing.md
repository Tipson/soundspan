...
## Bounded signal projection

The earlier CPU profile also identified significant Prisma relation assembly, including repeated playback/collection track joins. `loadSignalsFromPrisma` now projects exactly the selected track fields with PostgreSQL `row_to_json`, retaining the original parent filters, ordering and limits: 1,000 plays and 2,000 likes/playlist entries. Missing related tracks remain null before the existing filtering step. Settings, signal interpretation and ranking are unchanged. No relationJoins preview flag or global ORM behavior was enabled.
